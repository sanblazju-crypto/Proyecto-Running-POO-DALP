"""Seed: populates DB with sample data. Run: python seed.py"""
...
async def seed():
    ...
    async with AsyncSessionLocal() as db:
        users = []
        for u in USERS:
            user = User(email=u["email"], username=u["username"], full_name=u["full_name"],
        ...
        for i, user in enumerate(users):
            for j in range(3):
                ...
                dist = 10000 + j * 2000
                dur  = 3600 + j * 600
                act  = Activity(user_id=user.id, title=f"Entrenamiento {j+1}",
                                discipline=user.disciplines[0],
                                activity_type=ActivityType.TRAINING, started_at=started,
                                duration_seconds=dur, distance_meters=dist,
                                avg_pace_sec_per_km=dur/(dist/1000),
                                avg_speed_kmh=(dist/1000)/(dur/3600),
                                is_public=True)
                db.add(act)
                db.add(Post(author_id=user.id, post_type=PostType.ACTIVITY,
                            content=f"Completé entrenamiento · {dist/1000:.1f} km",
                            is_public=True, likes_count=j*3))
