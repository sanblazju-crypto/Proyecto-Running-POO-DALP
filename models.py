...
class Event(Base):
    ...
    organizer_contact = Column(String(200))
    ...
    __table_args__ = (
        ...
        Index("ix_events_discipline_date", "discipline", "date"),
    )
...
class Activity(Base):
    ...
    duration_seconds = Column(Integer)           # total elapsed time
    moving_time_seconds = Column(Integer)        # time actually moving
    ...
    perceived_effort = Column(Integer)           # 1-10 RPE
    weather_conditions = Column(String(100))
    temperature_celsius = Column(Float)
    gpx_url = Column(String(500))
    strava_activity_id = Column(String(50), nullable=True, index=True)
    splits = Column(JSON, default=list)          # per-km split data
    lap_data = Column(JSON, default=list)
    notes = Column(Text)
    ...
    __table_args__ = (
        Index("ix_activities_user_date", "user_id", "started_at"),
    )
...
class EventResult(Base):
    ...
    __table_args__ = (
        UniqueConstraint("event_id", "user_id", name="uq_event_result_user"),
    )
...
class PersonalBest(Base):
    ...
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    discipline = Column(Enum(Discipline), nullable=False)
    distance_label = Column(String(30), nullable=False)  # e.g. "5K", "10K", "Half Marathon"
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "discipline", "distance_label", name="uq_pb_user_discipline_distance"),
    )
...
class TrainingSession(Base):
    ...
    target_heart_rate_zone = Column(Integer)  # 1-5
    target_pace_sec_per_km = Column(Float)
    warmup_description = Column(Text)
    main_set_description = Column(Text)
    cooldown_description = Column(Text)
    notes = Column(Text)
...
class PlanAssignment(Base):
    ...
    __table_args__ = (
        UniqueConstraint("plan_id", "athlete_id", name="uq_plan_assignment"),
    )
...
class Notification(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    type = Column(String(50), nullable=False)  # follow, like, comment, team_invite, plan_assigned
